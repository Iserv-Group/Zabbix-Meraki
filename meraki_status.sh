...
IFS=$'\n' orgs=(`cat $input_folder"meraki_networks.json" | jq '.[] | .org_name' | sort | uniq | sed 's/\"//g'`) #List of Meraki orginizations to monitor seperated by new lines. 
#Increment through each org on the list
status(){
	i=$1	
	api_key=$2
	#Grabs the Org ID
	org_id=$(cat $input_folder"meraki_networks.json" | jq --arg var1 "$i" '.[] |  select(.org_name == $var1 ).org_id' | sed 's/\"//g' | uniq)
	if [ ! -z "$org_id" ]; then
		...
		devices=$(cat $input_folder"meraki_networks.json" | jq --arg var1 "$org_id" '.[] | select(.org_id == $var1 )' | jq -s)
...
cat devices.tmp | jq > $output_folder"meraki.json"
rm devices.tmp
